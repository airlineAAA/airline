...
$rowcount = mysqli_num_rows($result);
    ...
    if($rowcount == 0){
        echo "<div class='alert alert-info'><strong>Брони нет.</strong></div>";
    }
    else{
    echo "<div class='alert alert-info'>Ваши бронирования:</div>";
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        echo '<td>
            ...
            <button type="submit" class="btn btn-danger">Удалить</button></div>
            </form>        
            </td>';
        }else{
            echo "<td><button type='button' class='btn btn-warning' onclick='myFunction()'>Сейчас нет свободных мест</button></td>";
        }
    ...
    echo "<div class='row'>
			  ...
			  <div class='col-sm-4'><p class='lead'>Всего: <span id='total'>$".$total."</span></p></div>
			  <div class='col-sm-4'><button type='submit' class='btn btn-primary'>Оплатить</button></div>
			</div>";
